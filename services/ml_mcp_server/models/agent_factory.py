...
# --- Mocking external schemas for standalone execution ---
...
class Prediction(BaseModel):
    ...
    xai_scores: List[XAIScore]
    metadata: ModelMetadata

class MLModel:
    """Base mock class for MLModel."""
    pass
# ---------------------------------------------------------
...
class DynamicMLAgent(MLModel):
    ...
    def _build_dynamic_schema(self) -> Type[BaseModel]:
        ...
        fields = {
            self._sanitize(name): (float, Field(description=f"Feature measurement: {name}"))
    ...
    def predict(self, inputs: BaseModel) -> Prediction:
        ...
        row_dict = {name: data[self._sanitize(name)] for name in self.config.feature_names}
        ...
        all_scores = [
            XAIScore(
                ...
                feature_value=float(X_df.iloc[0, i]),
            )
...
from .ml_agent_factory import AgentConfig, DynamicMLAgent
