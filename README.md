...
2. **Selects and runs niche ML predictors** from a catalog of 11 tabular clinical models — each returning a prediction, confidence score, and SHAP-style feature importance
...
│        │ │ Agent   │ │                    │
│ 11 XGB │ │BeeAI    │ │ SQLite seed DB     │
│ models │ │sub-agent│ │ + CSV upload       │
│ + SHAP │ │ (LLM)   │ │ tool               │
└────────┘ └─────────┘ └────────────────────┘
```
...
- Holds the ML catalog (dynamically injected into the system prompt at startup) and decides which predictors to call based on expert input
...
- Attaches live web search results as `Citation` objects via Tavily (configurable domain whitelist: MedlinePlus, PubMed by default)
...
4. The registry auto-discovers it on next server start; the orchestrator gains the tool via MCP with the catalog description injected into its system prompt
